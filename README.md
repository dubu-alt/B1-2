...
| 기준 | 평가 항목 | README 반영 위치 | 상태 |
| --- | --- | --- | --- |
| 1 | OOM: 메모리 사용량이 선형 증가하다가 프로세스 강제 종료 | OOM 리포트 > 증거 | 충족 |
| 2 | OOM: `MEMORY_LIMIT` 조정 전후 생존 시간 비교 | OOM 리포트 > 조치 및 검증 | 충족 |
| 3 | CPU: CPU 임계치 초과 패턴과 종료 여부 | CPU 리포트 > 증거 | 보완: 실제 로그상 강제 종료가 아니라 cooldown으로 생존 |
| 4 | CPU: `CPU_MAX_OCCUPY` 조정 전후 종료 여부/생존 시간 비교 | CPU 리포트 > 조치 및 검증 | 충족 |
| 5 | Deadlock: PID는 있으나 CPU/메모리 변화 없이 로그 멈춤 | Deadlock 리포트 > 증거 | 충족 |
| 6 | Deadlock: `MULTI_THREAD_ENABLE` 조정 전후 재현/회피 비교 | Deadlock 리포트 > 조치 및 검증 | 충족 |
| 7 | Format: 3개 리포트 모두 GitHub Issue 구조 | 각 장애 리포트 | 충족 |
| 8 | Evidence: PID, 타임스탬프, 핵심 로그 메시지 포함 | 각 장애 리포트 > 증거 | 충족 |
| 9~20 | 진단 도구, 판단 흐름, 원리, 운영 개선, 회고 | 평가항목 상세 답변 | 충족 |
